            ...
            device = (await SerialPort.list()).find((device) => {
                if (isDevelopment) {
                    return this.allowedDevices.find((allowedDevice) => {
                        if (allowedDevice.vid === device.vendorId && allowedDevice.pid === device.productId) {
                            this.connectedDevice = allowedDevice;
                            return true;
                        }
                        else {
                            return false;
                    ...
                    return this.allowedDevices.find((allowedDevice) => {
                        if (allowedDevice.vid === device.vendorId && allowedDevice.pid === device.productId && allowedDevice.isProduction) {
                            this.connectedDevice = allowedDevice;
                            return true;
                        }
                        else {
